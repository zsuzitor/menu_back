...
namespace TaskManagementApp.Models.Services
{
    ...
    public sealed class TaskManagementAppEmailService : EmailServiceBase, ITaskManagementAppEmailService
    {
        ...
        {
            var config = await _configurationService.GetAsync(Consts.EmailConfigurationsCode.NewExecutorInTask);
            var text = config.Value.Replace("{{taskName}}", taskName ?? "")
                .Replace("{{taskUrl}}", taskUrl);
            await QueueEmailAsync(email, DefaultSubject, text);
        }
