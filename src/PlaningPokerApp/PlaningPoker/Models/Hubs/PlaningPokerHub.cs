...
namespace PlaningPoker.Models.Hubs
{
    ...
    public class PlaningPokerHub : Hub
    {
        ...
        private readonly IPlaningApiHelper _apiHealper;
        ...
        public PlaningPokerHub(MultiThreadHelper multiThreadHelper,
            IStringValidator stringValidator, IPlaningPokerService planingPokerService,
            IUserService userService,
            IPlaningApiHelper apiHealper, IJWTService jwtService, IHasher hasher, IErrorService errorService
            , IConfigurationService configurationService, ILoggerFactory loggerFactory)
        {
            ...
            _hublogger = loggerFactory.CreateLogger(Constants.Loggers.PlaningPoker);
        ...
        public async Task CreateRoom(string roomName, string password, string username)
        {
            ...
            await _apiHealper.DoStandartSomething(async () =>
            {
                ...
                if (expired && userInfo != null)
                {
                    ...
                    await Clients.Caller.SendAsync(Constants.PlaningPokerHubEndpoints.NeedRefreshTokens);
                }

                var user = new PlaningUser()
                {
                ...
                var room = await _planingPokerService.CreateRoomWithUserAsync(roomName, password, user);
                if (room == null)
                {
                    await Clients.Caller.SendAsync(Constants.PlaningPokerHubEndpoints.RoomNotCreated);
        ...
        public async Task AliveRoom(string roomName)
        {
            ...
            await _apiHealper.DoStandartSomething(async () =>
            {
                var room = await _planingPokerService.TryGetRoomAsync(roomName);
                if (room == null)
                {
                    throw new SomeCustomException(Constants.PlaningPokerErrorConsts.RoomNotFound);
                }

                var newDieDate = await _planingPokerService.AddTimeAliveRoom(room);
                if (newDieDate != DateTime.MinValue)
                {
                    await Clients.Group(roomName).SendAsync(Constants.PlaningPokerHubEndpoints.NewRoomAlive, newDieDate);
                    return;
                }
        ...
        public async Task SetRoomCards(string roomName, string cards)
        {
            ...
            await _apiHealper.DoStandartSomething(async () =>
            {
                ...
                {
                    throw new SomeCustomException(Constants.PlaningPokerErrorConsts.RoomBadVoteMarks);
                }
                ...
                var rs = await _planingPokerService.SetRoomCards(room, connectionId, cardsList);
                if (!rs)
                {
                    throw new SomeCustomException(ErrorConsts.SomeError);
                }

                await Clients.Group(roomName).SendAsync(Constants.PlaningPokerHubEndpoints.RoomCardsChanged, cardsList);
                var result = await _planingPokerService.RecalcEndVoteInfo(room);
                if (result == null)
                {
                    throw new SomeCustomException(ErrorConsts.SomeError);
                }
                ...
                //todo зачем это?
                await Clients.Group(roomName).SendAsync(Constants.PlaningPokerHubEndpoints.VoteEnd, result);
        ...
        public async Task EnterInRoom(string roomName, string password, string username)
        {
            ...
            await _apiHealper.DoStandartSomething(async () =>
            {
                ...
                {
                    _errorService.AddError(Constants.PlaningPokerErrorConsts.RoomNameIsEmpty, 
                        (await _configurationService.GetAsync(Constants.PlaningPokerErrorConsts.RoomNameIsEmpty)).Value);
                    await _apiHealper.NotifyFromErrorService();
                    await Clients.Caller.SendAsync(Constants.PlaningPokerHubEndpoints.ConnectedToRoomError);
                }

                var room = await _planingPokerService.TryGetRoomAsync(roomName, password);
                if (room == null)
                {
                    throw new SomeCustomException(Constants.PlaningPokerErrorConsts.RoomNotFound);
                }
        ...
        public async Task StartVote(string roomName)
        {
            ...
            await _apiHealper.DoStandartSomething(async () =>
            {
                await _planingPokerService.StartVoteAsync(roomName, connectionId);
                //todo Consts.PlaningPokerHubEndpoints.ConnectedToRoomError

                await Clients.Group(roomName).SendAsync(Constants.PlaningPokerHubEndpoints.VoteStart);
        ...
        public async Task EndVote(string roomName)
        {
            ...
            await _apiHealper.DoStandartSomething(async () =>
            {
                var result = await _planingPokerService.EndVoteAsync(roomName, connectionId);
                if (result == null)
                {
                    throw new SomeCustomException(ErrorConsts.SomeError);
                }
        ...
        public async Task<bool> Vote(string roomName, string vote)
        {
            ...
            return await _apiHealper.DoStandartSomething(async () =>
            {
                ...
                (bool sc1, string userId) = await _planingPokerService.ChangeVote(room, connectionId, vote);
                if (!sc1)
                {
                    throw new SomeCustomException(ErrorConsts.SomeError);
                }

                //var allVoted = await _planingPokerService.AllVoted(room);
                var adminsId = await _planingPokerService.GetAdminsIdAsync(room);
                if (adminsId != null && adminsId.Count > 0)
                {
                    await Clients.Clients(adminsId).SendAsync(Constants.PlaningPokerHubEndpoints.VoteChanged, userId, vote);
                }

                //if (allVoted)
                //{
                //    var erF = new ErrorObjectReturnFactory();
                //    await Clients.Clients(adminsId).SendAsync(Consts.PlaningPokerHubEndpoints.NotifyFromServer,
                //        erF.GetObjectReturn(new ErrorObject(){Errors = new List<OneError>(){new OneError(Consts.PlaningPokerNotifyConsts.AllAreWoted, "все участники проголосовали") }}));
                //}

                await Clients.GroupExcept(roomName, adminsId)
                    .SendAsync(Constants.PlaningPokerHubEndpoints.VoteChanged, userId, "?");
        ...
        public async Task KickUser(string roomName, string userId)
        {
            ...
            await _apiHealper.DoStandartSomething(async () =>
            {
                var kicked = await _planingPokerService.KickFromRoomAsync(roomName, connectionId, userId);

                if (kicked.sc)
                {
                    await Clients.Group(roomName)
                        .SendAsync(Constants.PlaningPokerHubEndpoints.UserLeaved, new List<string>() { userId });
        ...
        public async Task<bool> UserNameChange(string roomName, string newUserName)
        {
            ...
            return await _apiHealper.DoStandartSomething(async () =>
            {
                ...
                (bool sc, string userId) =
                    await _planingPokerService.ChangeUserNameAsync(roomName, connectionId, newUserName);

                if (sc)
                {
                    await Clients.Group(roomName)
                        .SendAsync(Constants.PlaningPokerHubEndpoints.UserNameChanged, userId, newUserName);
                    return true;
                }
        ...
        public async Task AddNewRoleToUser(string roomName, string userId, string newRole)
        {
            ...
            await _apiHealper.DoStandartSomething(async () =>
            {
                var sc = await _planingPokerService.AddNewRoleToUserAsync(roomName, userId, newRole, connectionId);
                if (!sc)
                {
                    throw new SomeCustomException(ErrorConsts.SomeError);
                }
                ...
                await Clients.Group(roomName)
                    .SendAsync(Constants.PlaningPokerHubEndpoints.UserRoleChanged, userId, 1, newRole);
                return;
        ...
        public async Task RemoveRoleUser(string roomName, string userId, string oldRole)
        {
            ...
            await _apiHealper.DoStandartSomething(async () =>
            {
                var sc = await _planingPokerService.RemoveRoleUserAsync(roomName, userId, oldRole, connectionId);
                if (sc)
                {
                    await Clients.Group(roomName)
                        .SendAsync(Constants.PlaningPokerHubEndpoints.UserRoleChanged, userId, 2, oldRole);
                }
            }, _logger);
        ...
        public async Task AddNewStory(string roomName, string storyName, string storyDescription)
        {
            ...
            await _apiHealper.DoStandartSomething(async () =>
            {
                ...
                var sc = await _planingPokerService.AddNewStoryAsync(roomName, connectionId, newStory);

                if (sc)
                {
                    await Clients.Group(roomName)
                        .SendAsync(Constants.PlaningPokerHubEndpoints.AddedNewStory, new StoryReturn(newStory));
                }
            }, _logger);
        ...
        public async Task ChangeCurrentStory(string roomName, string storyId, string storyName, string storyDescription)
        {
            ...
            await _apiHealper.DoStandartSomething(async () =>
            {
                ...
                {
                    throw new SomeCustomException(Constants.PlaningPokerErrorConsts.StoryNotFound);
                }

                var sc = await _planingPokerService.ChangeStoryAsync(roomName, connectionId, newStory);

                if (sc)
                {
                    await Clients.Group(roomName)
                        .SendAsync(Constants.PlaningPokerHubEndpoints.CurrentStoryChanged, storyId, storyName,
        ...
        public async Task MakeCurrentStory(string roomName, string storyId)
        {
            ...
            await _apiHealper.DoStandartSomething(async () =>
            {
                var sc = await _planingPokerService.ChangeCurrentStoryAsync(roomName, connectionId, storyId);

                if (sc)
                {
                    await Clients.Group(roomName).SendAsync(Constants.PlaningPokerHubEndpoints.NewCurrentStory, storyId);
                }
            }, _logger);
        ...
        public async Task DeleteStory(string roomName, string storyId)
        {
            ...
            await _apiHealper.DoStandartSomething(async () =>
            {
                var sc = await _planingPokerService.DeleteStoryAsync(roomName, connectionId, storyId);

                if (sc)
                {
                    await Clients.Group(roomName).SendAsync(Constants.PlaningPokerHubEndpoints.DeletedStory, storyId);
                }
            }, _logger);
        ...
        public async Task MakeStoryComplete(string roomName, string storyId)
        {
            ...
            await _apiHealper.DoStandartSomething(async () =>
            {
                var (oldId, story) = await _planingPokerService.MakeStoryCompleteAsync(roomName, storyId, GetConnectionId());
                if (story != null)
                {
                    await Clients.Group(roomName).SendAsync(Constants.PlaningPokerHubEndpoints.MovedStoryToComplete, oldId,
        ...
        public async Task OnWindowClosedAsync(string roomName)
        {
            ...
            await _apiHealper.DoStandartSomethingWithoutResponse(async () =>
            {
                ...
                {
                    (bool sc, string userId) = await _planingPokerService.LeaveFromRoomAsync(roomName, connectionId);
                    if (sc)
                    {
                        await Groups.RemoveFromGroupAsync(connectionId, roomName);
                        await Clients.Group(roomName).SendAsync(Constants.PlaningPokerHubEndpoints.UserLeaved,
        ...
        public async Task<bool> SaveRoom(string roomName)
        {
            ...
            return await _apiHealper.DoStandartSomething(
                async () =>
                {
                    var res = await _planingPokerService.SaveRoomAsync(roomName, connectionId);
                    if (res.Success)
                    {
                        await Clients.Group(roomName)
                         .SendAsync(Constants.PlaningPokerHubEndpoints.RoomWasSaved, res);
        ...
        public async Task DeleteRoom(string roomName)
        {
            ...
            await _apiHealper.DoStandartSomething(async () =>
            {
                var room = await _planingPokerService.DeleteRoomAsync(roomName, connectionId);
        ...
        public override async Task OnDisconnectedAsync(Exception exception)
        {
            ...
            //    (bool sc, string userId) = await _planingPokerService.LeaveFromRoom(roomName, userConnectionId);
            //    if (sc)
            //    {
            //        await Groups.RemoveFromGroupAsync(userConnectionId, roomName);
            //        await Clients.Group(roomName).SendAsync(Consts.PlaningPokerHubEndpoints.UserLeaved,
            //            new List<string>() {userId});
        ...
        /// <returns></returns>
        private async Task<bool> EnterInRoom(Room room, PlaningUser user)
        {
            ...
            var (sc, oldConnectionId) = await _planingPokerService.AddUserIntoRoomAsync(room, user);
            if (!sc)
            {
                throw new SomeCustomException(ErrorConsts.SomeError);
            }
            ...
            var returnUser = new PlaningUserReturn(us.res);

            await Clients.Group(roomnm.res).SendAsync(Constants.PlaningPokerHubEndpoints.NewUserInRoom, returnUser);
            await Groups.AddToGroupAsync(userConnectionId, roomnm.res);

            await Clients.Caller.SendAsync(Constants.PlaningPokerHubEndpoints.EnteredInRoom, userId,
        ...
        private void ValidateRoomName(string roomName)
        {
            ...
            {
                throw new SomeCustomException(Constants.PlaningPokerErrorConsts.BadRoomNameWithRoomCreating);
            }
        }
        ...
        private void ValidateUserName(string userName)
        {
            ...
            {
                throw new SomeCustomException(Constants.PlaningPokerErrorConsts.UsernameBad);
            }
        }
        ...
        private void Log(LogLevel lvl, string action, string message, string roomName, string connectionId, string userId)
        {
            ...
            config.Add("action", $"PlaningPoker-{action}");
