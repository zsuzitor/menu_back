...
{
    public sealed class PlaningPokerService : IPlaningPokerService
    {
        ...
        public async Task<List<PlaningUser>> GetAllUsersWithRight(Room room, string userConnectionId)
        {
            var users = await GetAllUsers(room);
            if (users == null || users.Count == 0)
            {
                return new List<PlaningUser>();
            }
        ...
        public async Task<List<PlaningUser>> GetAllUsersWithRightAsync(string roomName, string userConnectionId)
        {
        ...
        public async Task<RoomInfoReturn> GetRoomInfoWithRight(Room room, string userConnectionId)
        {
            ...
            {
                throw new SomeCustomException(Constants.PlaningPokerErrorConsts.PlaningUserNotFound);
            }
        ...
        public async Task<Room> DeleteRoomAsync(string roomName, string userConnectionIdRequest)
        {
            ...
            if (!isAdmin)
            {
                throw new SomeCustomException(Constants.PlaningPokerErrorConsts.DontHaveAccess);
        ...
        public async Task<(bool sc, string oldConnectionId)> AddUserIntoRoomAsync(string roomName, PlaningUser user)
        {
            var room = await TryGetRoomAsync(roomName);
            return await AddUserIntoRoomAsync(room, user);
        }

        public async Task<(bool sc, string oldConnectionId)> AddUserIntoRoomAsync(Room room, PlaningUser user)
        {
        ...
        public async Task<(bool sc, string userId)> ChangeVote(Room room, string connectionUserId, string vote)
        {
            ...
            var suc = await room.SetConcurentValue(_multiThreadHelper, rm =>
            {
                ...
                {
                    throw new SomeCustomException(Constants.PlaningPokerErrorConsts.CantVote);
                }

                if (rm.StoredRoom.Cards.FirstOrDefault(x => x?.Equals(vote) ?? false) == null)
                {
                    throw new SomeCustomException(Constants.PlaningPokerErrorConsts.RoomBadVoteMark);
                }
        ...
        public async Task<Room> CreateRoomWithUserAsync(string roomName, string password, PlaningUser user)
        {
            if (string.IsNullOrWhiteSpace(roomName))
            {
                throw new SomeCustomException(Constants.PlaningPokerErrorConsts.RoomNameIsEmpty);
            }
            ...
            {
                throw new SomeCustomException(Constants.PlaningPokerErrorConsts.RoomAlreadyExist);
            }
            ...
            throw new SomeCustomException(Constants.PlaningPokerErrorConsts.SomeErrorWithRoomCreating);
        }
        ...
        /// <returns></returns>
        public async Task<List<PlaningUser>> GetAllUsers(Room room)
        {
            if (room == null)
            {
                throw new SomeCustomException(Constants.PlaningPokerErrorConsts.RoomNotFound);
        ...
        /// <returns></returns>
        public async Task<List<PlaningUser>> GetAllUsersAsync(string roomName)
        {
            var room = await TryGetRoomAsync(roomName);
        ...
        public async Task<bool> DeleteStoryAsync(string roomName, string userConnectionIdRequest, string storyId)
        {
            return await UpdateIfCan(roomName, userConnectionIdRequest, true, async (room) =>
            {
                ...
                {
                    throw new SomeCustomException(Constants.PlaningPokerErrorConsts.StoryBadStatus);
                }
        ...
        public async Task<(PlaningUser user, bool sc)> KickFromRoomAsync(string roomName, string userConnectionIdRequest,
            string userId)
        {
            var room = await TryGetRoomAsync(roomName);
        ...
        public async Task<(PlaningUser user, bool sc)> KickFromRoomAsync(Room room, string userConnectionIdRequest,
            string userId)
        {
        ...
        public async Task<bool> ChangeRoomPasswordAsync(string roomName, string userConnectionId, string oldPassword, string newPassword)
        {
            ...
            var sc = await UpdateIfCan(room, userConnectionId, true, async rm =>
            {
                ...
                {
                    throw new SomeCustomException(Constants.PlaningPokerErrorConsts.RoomBadPassword);
                }
        ...
        public async Task<bool> SetRoomCards(Room room, string userConnectionId, List<string> cards)
        {
            ...
            {
                throw new SomeCustomException(Constants.PlaningPokerErrorConsts.RoomBadCountCards);
            }

            if (cards.FirstOrDefault(x => x.Length > 5) != null)
            {
                throw new SomeCustomException(Constants.PlaningPokerErrorConsts.RoomBadLengthCard);
            }
        ...
        private async Task<bool> UpdateIfCan(Room room, Predicate<PlaningUser> userComparer
            , bool isAdmin,
        Func<StoredRoom, Task<bool>> workWithRoom)
        {
        ...
        private async Task<bool> UpdateUserIfCan(string roomName, string userId,
            string userConnectionIdRequest,
            Func<PlaningUser, bool> userChange)
        {
        ...
        private List<PlaningUser> ClearHideData(RoomSatus roomStatus, string currentUserConnectionId,
            List<PlaningUser> users)
        {
            if (users == null)
            {
                return null;
