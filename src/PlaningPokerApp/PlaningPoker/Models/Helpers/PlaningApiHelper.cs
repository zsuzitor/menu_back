...
namespace PlaningPoker.Models.Helpers
{
    public interface IPlaningApiHelper : IApiHelper
    {
    ...
    public sealed class PlaningApiHelper : ApiHelper, IPlaningApiHelper
    {
        private Hub _planingHub;

        public PlaningApiHelper(IErrorService errorService, IConfigurationService configurationService,
        ...
        public async Task<T> DoStandartSomethingWithoutResponse<T>(Func<Task<T>> action, T defaultResult, ILogger logger)
        {
            ...
            {//ветка вообще не особо актуальная для покера
                ...
                var error2 = await _configurationService.GetAsync(Constants.PlaningPokerErrorConsts.PlaningUserNotFound);
                _errorService.AddError(Constants.PlaningPokerErrorConsts.PlaningUserNotFound, error2.Value);
        ...
        public async Task<bool> NotifyFromErrorService()
        {
            ...
            await _planingHub.Clients.Caller.SendAsync(Constants.PlaningPokerHubEndpoints.NotifyFromServer,
        ...
        {
            if (_errorService.HasError(Constants.PlaningPokerErrorConsts.RoomNotFound))
            {
                //пока так
                //можно в хабе делать доп try catch и уже там что то такое добавить а тут убрать
                await _planingHub.Clients.Caller.SendAsync(Constants.PlaningPokerHubEndpoints.ConnectedToRoomError);
            }
        }
